...
public class MinecraftFontRenderer extends CFont {
    ...
    public void drawStringWithClientColor(String text, double x, double y, int opacity, boolean shadow) {
        ...
        for(char textChar : text.toCharArray()) {
            ...
            if (!hasReachedSS) {
                if(shadow) {
                    this.drawStringWithShadow(tmp, xTmp, y, ColorUtil.getClientColor(i, opacity).getRGB());
                }else {
                    this.drawString(tmp, xTmp, y, ColorUtil.getClientColor(i, opacity).getRGB());
                }
        ...
        for (int index = 0; index < text.length(); index++) {
            ...
            if (character == '\u00a7') {
                ...
                if (colorIndex < 16) {
                    ...
                    setColor(this.colorCode[colorIndex], alpha);
                } else {
                    switch (colorIndex) {
                        default:
                            setColor(color);
