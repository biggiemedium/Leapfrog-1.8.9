    ...
    public static Color getClientColor(int index, int alpha) {
        for(AccentColor c : LeapFrog.colorManager.getColors()) {
            if(c.equals(LeapFrog.colorManager.getClientColor())) {
                return interpolateColorsBackAndForth(15, index, new Color(c.getMainColor().getRed(), c.getMainColor().getGreen(), c.getMainColor().getBlue(), alpha), new Color(c.getAlternativeColor().getRed(), c.getAlternativeColor().getGreen(), c.getAlternativeColor().getBlue(), alpha), false);
            }
        }
